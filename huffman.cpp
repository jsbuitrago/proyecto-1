...
/**esta funcion es usada para guarda los archivos que se van generando para mas adelante descomprimirlos*/
...
/**igual que en la cabecera esta funcions pes para hacer lectura del archivo a decodificar*/
...
unsigned int ARCH_IN::Byte()
{
	...
	if((!indice)&&(!feof(Arch)))//test del codigo
		return fgetc(Arch);
	else
	{
		res = fgetc(Arch);
...
/**es para hacer la salida del archivo */
...
/**se realiza la compresion del archivo*/
...
/**funcioes de movimiento para la creacion del arbol*/
bool HOJA::MoverADer()
{
	if(!HojaDer())
		return false;
